...
exports.getOrders= async (req,res) => {
  ...
  try {
      ...
      const query =`SELECT sr.completed_date as completed_date ,sr.description as description ,(select name from cities c where sr.city_id =c.city_id) as city,(select CONCAT(firstName, ' ', lastName)  from serviceproviders where sp_id=sr.sp_id) as sp_name,s.name as name ,sr.address as address,sr.request_date as request_date,sr.status as status,sr.request_id as request_id from servicerequests sr join services s on  s.service_id = sr.service_id 
